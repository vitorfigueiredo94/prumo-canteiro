...
async function main() {
  ...
  const t1 = await prisma.terreno.upsert({
    ...
    create: {
      id: "terreno-1",
      empresaId: empresa.id,
      nome: "Lote 01 — Residencial Pinheiros",
      numero: "LT-01",
      endereco: "Rua das Acácias, 100",
      cidade: "São Paulo",
      area: 360.0,
      status: "em_obra",
      aquisicao: new Date("2024-03-15"),
      valorCompra: 280000.0,
    },
  });
  ...
  const t2 = await prisma.terreno.upsert({
    ...
    create: {
      id: "terreno-2",
      empresaId: empresa.id,
      nome: "Lote 07 — Condomínio Serra Verde",
      numero: "LT-07",
      endereco: "Estrada da Serra, km 12",
      cidade: "Guarulhos",
      area: 520.0,
      status: "disponivel",
      aquisicao: new Date("2024-06-01"),
  ...
  const t3 = await prisma.terreno.upsert({
    ...
    create: {
      id: "terreno-3",
      empresaId: empresa.id,
      nome: "Lote 12 — Centro Comercial",
      numero: "LT-12",
      endereco: "Av. Paulista, 2000",
      cidade: "São Paulo",
      area: 180.0,
      status: "vendido",
      aquisicao: new Date("2023-11-10"),
      valorCompra: 450000.0,
    },
  });
  ...
  for (const fid of [f1.id, f2.id, f3.id]) {
    await prisma.alocacao.upsert({
      where: { funcionarioId_obraId: { funcionarioId: fid, obraId: o1.id } },
      update: {},
      create: { funcionarioId: fid, obraId: o1.id },
    });
  }
  ...
  const notas = [
    {
      id: "nf-1",
      fornecedor: "Madeiras São João Ltda.",
      numero: "001234",
      categoria: "material" as const,
      valor: 12500.0,
      emissao: new Date("2024-05-10"),
      status: "confirmada" as const,
      descricao: "Madeira para estrutura do telhado",
    },
    {
      id: "nf-2",
      fornecedor: "Eletro Construções ME",
      numero: "005678",
      categoria: "servicos" as const,
      valor: 8200.0,
      emissao: new Date("2024-06-05"),
      status: "confirmada" as const,
      descricao: "Instalação elétrica fase 1",
    },
    {
      id: "nf-3",
      fornecedor: "Cimento Forte Distribuidora",
      numero: undefined,
      categoria: "material" as const,
      valor: 5800.0,
      emissao: new Date("2024-07-20"),
  ...
  // ── Parcelas ─────────────────────────────────────────────────────────────
  for (let i = 0; i < 12; i++) {
    const venc = new Date(2024, 8 + i, 1);
    await prisma.parcela.upsert({
      ...
      create: {
        ...
        valor: 41666.67,
        pago: i < 3,
        dataPag: i < 3 ? new Date(2024, 8 + i, 3) : null,
      },
    });
  }
